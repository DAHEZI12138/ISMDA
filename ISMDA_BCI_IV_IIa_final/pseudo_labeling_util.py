...
def pseudo_labeling(args, data_loader, itr, save_dir, save_dir_models, z, round_idx):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, indexs, _) in enumerate(data_loader):
            ...
            for _ in range(f_pass):
                ...
                feature_extractor = EEGNet_ATTEN(EEGNet_ATTEN_configs, round(random.uniform(0.4, 0.6), 1)).float().to(
                    args.device)
            ...
            max_std = out_std.gather(1, max_idx.view(-1,1))  # The variance corresponding to the maximum value

            idx_list.extend(indexs.numpy().tolist())
            gt_list.extend(targets.cpu().numpy().tolist())  # original label
            target_list.extend(max_idx.cpu().numpy().tolist())  # position of the maximum value
    ...
    if itr < PACC_configs.class_blnc - 1:
        ...
        for class_idx in range(configs.num_classes):
            class_len = len(np.where(pseudo_target == class_idx)[0])  # counting the number of samples in each category of pseudo-labelling
        ...
        for class_idx in range(configs.num_classes):
            ...
            if len(np.where(pseudo_target == class_idx)[0]) > 0:
                ...
                current_class_idx = current_class_idx[0][
                    sorted_maxstd_idx[:min_count]]  # select the samples with minimal uncertainty
    ...
    pseudo_label_dict = {'softmax_labels': pseudo_idx, 'labels': pseudo_target,
                         'samples': torch.from_numpy(target_data)}
