...
def cross_domain_test_fea(src_test_dl, tgt_test_dl, device, save_dir, save_dir_features, z, save_dir_models):
    # Prepare and save the best model features
    ...
    feature_extractor = EEGNet_ATTEN(EEGNet_ATTEN_configs, 0.5).float().to(device)
    ...
    with torch.no_grad():
        for data, labels in valid_dl:
            ...
            if not features:
                ...
                pred = predictions.max(1, keepdim=False)[1]  # get the index of the max log-probability
