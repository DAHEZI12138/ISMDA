...
class EEGNet_ATTEN(nn.Module):
    def __init__(self, configs, DOR): # DOR adjustable for generation of sub-networks
    ...
    def _make_layer(self, block, planes, blocks, expansion, stride=1):  # makes residual SE block
        downsample = None
...
class AttenBlock(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        y = self.fc3(self.avg_pool(x).view(b, c)).view(b, 4, 1, 1, 1)# branch attention
        y = self.sf(y)
