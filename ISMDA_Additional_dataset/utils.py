...
def plot_feature_map(configs, home_path, n, device, save_dir_features, save_dir_models):
    src_train_dl, trg_train_dl = data_generator_54(configs, n)

    cross_domain_test_fea(src_train_dl, trg_train_dl, device, home_path, save_dir_features, n, save_dir_models)
    # Read the features of subject n
    dataset = torch.load(os.path.join(home_path + save_dir_features, 'final_train_' + str(n) + '_to_' + str(n) + '_round_' + str(n) + '.pt'))
    ...
    name = os.path.join('features_54_' + str(n))
...
def copy_Files(destination, da_method):
    ...
    copy("dataloader/dataset54.py", os.path.join(destination_dir, "dataset54.py"))
    copy(f"models/models.py", os.path.join(destination_dir, f"models.py"))


def plot(x_train, y_train, x_test, y_test, colors, ax=None, title=None, draw_legend=True, draw_centers=True,
         draw_cluster_labels=True, legend_kwargs=None, label_order=None, **kwargs):
