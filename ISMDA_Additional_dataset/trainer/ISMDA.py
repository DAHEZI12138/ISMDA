...
import os
from dataloader.dataset54 import *
def cross_domain_train(src_id, trg_id, src_train_dl, trg_train_dl, Z, device, logger, configs, args):
    ...
    max_len = max(len(src_train_dl), len(trg_train_dl))
    args.max_iter = configs.num_epoch * max_len
    mem_fea = torch.rand(len(trg_train_dl.dataset), configs.target_features_length).to(device)
    mem_fea = mem_fea / torch.norm(mem_fea, p=2, dim=1, keepdim=True)
    mem_cls = torch.ones(len(trg_train_dl.dataset), class_num).to(device) / class_num

    optimizer_encoder = torch.optim.Adam(
        list(feature_extractor.parameters()) + list(classifier_1.parameters()), lr=configs.lr, betas=(configs.beta1, configs.beta2),
    ...
    for round_idx in range(param_config.self_training_iterations):
        ...
        pseudo_trg_train_dataset = torch.load(os.path.join(args.home_path, args.save_dir_pesudo_label1, file_name))
        ...
        if round_idx >= 1:
            ...
            pseudo_trg_train_dataset = torch.load(os.path.join(args.home_path, args.save_dir_pesudo_label2, file_name))
        ...
        for epoch in range(1, maxepoch + 1):
            ...
            for i, (src_data, src_labels) in enumerate(src_train_dl):
                ...
                if round_idx == 0:
                    eff = time0 / args.max_iter
                else:
                    eff = 1

                if (i + 1) % (53 *4) == 0:
                    try:
                        trg_data, pseudo_trg_labels, idx, _ = target_loader_iter.next()
                        ...
                        if round_idx == 0:
                            ...
                            dis = -torch.mm(trg_feat.detach(), mem_fea.t())
                            ...
                            for wi in range(w.size(0)):
                                    for wj in range(ATDOC_configs.K):
                                        w[wi][p1[wi, wj]] = 1 / ATDOC_configs.K
                            pred0 = w.mm(mem_cls)
                            weight_, pred = torch.max(w.mm(mem_cls), 1)
                            loss_ = nn.CrossEntropyLoss(reduction='none')(trg_pred, pred)
            ...
            if epoch % 1 == 0:
                target_loss, target_score, _, _ = model_evaluate((feature_extractor, classifier_1), trg_train_dl,
                                                                 device, False)
